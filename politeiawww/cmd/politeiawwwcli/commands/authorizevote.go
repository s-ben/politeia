...
Authorize or revoke proposal vote. Only the proposal author (owner of censorship token) 
can authorize or revoke vote. 

Arguments:
1. token      (string, required)   Proposal censorship token
2. action     (string, optional)   Valid actions are 'authorize' or 'revoke' (defaults to 'authorize')

Result:
{
  "action":               (string)  Action that was executed
  "receipt":              (string)  Server signature of client signature (signed token+version+action)
}`
